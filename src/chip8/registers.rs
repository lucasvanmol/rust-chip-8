...
pub struct Registers {
    ...
    DT: Arc<AtomicU8>, ST:  Arc<AtomicU8>   // Sound & Timer registers
}

impl Registers {
    pub fn new() -> Self {
        let r = Registers { PC: 0x200, SP: 0, I:0, Vx: [0; 16], DT: Arc::new(AtomicU8::new(0)), ST: Arc::new(AtomicU8::new(0)) };
        r.init();
        r
    }
    ...
    pub fn set_dt(&self, val: u8) {
        self.DT.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |_| Some(val)).unwrap();
    }
    ...
    pub fn set_st(&self, val: u8) {
        self.ST.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |_| Some(val)).unwrap();
    }
}